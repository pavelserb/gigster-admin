...
class AdminPanel {
  ...
  createVenuePhotoItem(photo, index) {
    ...
    // Get total photos count for button state
    const totalPhotos = this.config.event?.venue?.photos?.length || 0;
    
    item.innerHTML = `
      <div class="order-controls">
        <button class="btn btn-xs btn-secondary order-btn" 
                onclick="admin.moveVenuePhoto(${index}, 'up')" 
                ${index === 0 ? 'disabled' : ''} 
                title="Переместить вверх">
          ↑
        </button>
        <button class="btn btn-xs btn-secondary order-btn" 
                onclick="admin.moveVenuePhoto(${index}, 'down')" 
                ${index === totalPhotos - 1 ? 'disabled' : ''} 
                title="Переместить вниз">
          ↓
        </button>
      </div>
      <div class="venue-photo-content">
  ...
  moveVenuePhoto(index, direction) {
    const photos = this.config.event?.venue?.photos;
    if (!photos || index < 0 || index >= photos.length) {
      this.showError('Ошибка: фото не найдено');
      return;
    }
