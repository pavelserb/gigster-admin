...
# FTP деплой с сохранением пользовательских данных
echo "📤 Загружаю файлы через FTP с сохранением пользовательских данных..."
echo "⚠️  Пользовательские данные НЕ будут удалены:"
echo "   - translations.json"
echo "   - updates.json"
echo "   - pixels.json"
echo "   - config.json"
...
# Сначала загружаем все файлы (без удаления)
timeout 300 lftp -c "
set ssl:verify-certificate no;
set net:timeout 30;
set net:max-retries 3;
open -u $FTP_USER,$FTP_PASS $FTP_HOST;
cd $FTP_PATH;
mirror --reverse --verbose $BUILD_DIR .;
bye;
"

# Теперь удаляем только старые файлы сайта, НО НЕ пользовательские данные
echo "🧹 Удаляю старые файлы сайта (пользовательские данные сохраняются)..."
timeout 300 lftp -c "
set ssl:verify-certificate no;
set net:timeout 30;
set net:max-retries 3;
...
# Удаляем старые файлы сайта, но НЕ пользовательские данные
echo 'Удаляю старые CSS файлы...';
glob -a rm css/*.css;
echo 'Удаляю старые JS файлы...';
glob -a rm js/*.js;
echo 'Удаляю старые HTML файлы...';
glob -a rm *.html;
echo 'Удаляю старые серверные настройки...';
glob -a rm _server/*;
echo 'Удаляю старые мета файлы...';
glob -a rm meta/*;
echo 'Удаляю старые фавиконы...';
glob -a rm favicon/*;
