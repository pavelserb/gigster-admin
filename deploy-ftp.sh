...
cp admin.html "$BUILD_DIR/"
cp users.json "$BUILD_DIR/"
...
echo "   - config.json"
echo "   - Загруженные изображения"
echo "✅ Шрифты будут обновлены:"
echo "   - fonts/roboto-flex.woff2"

# Используем mirror с исключением пользовательских данных
echo "🔄 Синхронизирую файлы сайта (пользовательские данные сохраняются)..."
timeout 300 lftp -c "
set ssl:verify-certificate no;
set net:timeout 30;
set net:max-retries 3;
